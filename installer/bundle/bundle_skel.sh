...
install_extra_package()
{
    ...
    #            possibilities: python-ctypes, tar, sed
    ...
    local install_cmd=""
    local package=$1
    [ "$package" = "ctypes" ] && ctypes=0 || ctypes=1
...
if [ "$installMode" = "I" -o "$installMode" = "U" ]; then
    ...
    python_ctypes_installed
    if [ $? -ne 0 ]; then
        echo "Python ctypes library was not found, attempting to install python-ctypes..."
        install_extra_package ctypes
    fi
    if [ $? -ne 0 ]; then
        if [ -z "${forceFlag}" ]; then
            echo "Python ctypes library was not installed, installation cannot continue. Please" >&2
